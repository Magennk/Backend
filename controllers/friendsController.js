...
exports.sendFriendRequest = async (req, res) => {
  try {
    ...
    const result = await friendsModel.sendFriendRequest(senderEmail, recipientEmail);
    if (!result) {
...
exports.acceptFriendRequest = async (req, res) => {
  try {
    ...
    const result = await friendsModel.acceptFriendRequest(senderEmail, recipientEmail);
    if (!result) {
      return res.status(400).json({ message: "Failed to accept friend request" });
    }
...
exports.removeFriend = async (req, res) => {
  try {
    ...
    if (!result) {
      return res.status(400).json({ message: "Failed to remove friend or decline request" });
    }
